    ...
    <div class="row"><br>
        <div class="col-sm-12 col-lg-12 col-md-12 col-xs-12">
			...
			<div class="col-lg-8 text-center col-lg-offset-2">
				...
				$flag_font = False; //False = Standard, True = Specific
				$dol_utf = strlen($word);
				...
				if($flag_font == False){
					...
					function check_file_font($contents){
						...
						if($flag_font1 == True){
							echo "<script> alert('Upload a file in the specific font!');
								window.location ='search_a_word_in_file.php'; </script>";
						}
					}	
				?>
				...
				<tr style="background-color: #D0D0D0;">
					...
					<td>
						<?php
							...
							//echo "Word: " . $word . " " . $s1 . "<br>";
							
								foreach($razdeli_test as $str){
								
								$s = 0;
                                $test_niza = str_split($str);
                                foreach($test_niza as $kar){
                                    $s += ord($kar);
                                }
								//echo "Word: " . $str . " " . $s . "<br>";
								
								if(strcmp(strtolower($word), strtolower($str)) == 0){
									$counter1+=1;
								}
								else if(strcmp(strtolower($str), strtolower($word)) > 0){
                                    $s -= 617;
									//echo "Finalno: " . $s . "<br>";
                                    if($s == $s1)
                                        $counter1 += 1;
				...
				<tr style="background-color: #E0E0E0;">
					...
					<td>
						<?php
							$pom_word = mb_strtolower($word);
                            $niza = str_split($pom_word);
                            $nova_niza = array();
                           
                            for($i = 0; $i < count($niza); $i+=2){
                                $nova_niza[] = ($niza[$i] . $niza[$i+1]);
                            }
                           
                            $count_same = 0;
                            foreach($nova_niza as $n){
                                $count_same = substr_count($pom_word, $n)-1;
                            }
							
                            $count_diff = count($nova_niza) - $count_same;
                            echo $count_diff;
						?>
					</td>
				</tr>
				...
				<tr style="background-color: #F0F0F0;">
					...
					<td>
						<?php
							...
							$s = 0;
							//$test_niza = str_split($split_strings[1]); za docx 1 za pdf i txt 0
							if($txtHere == True){
								$test_niza = str_split($split_strings[0]);
								$nulti = $split_strings[0];
								//echo "FLEVA VO TXT";
							}
							else if($docxHere == True){
								$test_niza = str_split($split_strings[1]);
								$nulti = $split_strings[1];
							}
                            ...
                            $s1 = 0;
                            $test_niza1 = str_split(mb_strtolower($word));
                            foreach($test_niza1 as $kar1){
                                $s1 += ord($kar1);
                            }
								
							foreach($split_strings as $e){
								
								if($flagNulti == False){
									
									$flagNulti = True;
									if(strcmp(mb_strtolower($nulti), mb_strtolower($word)) == 0){
										if($docxHere == True){
											$counterBeginning += 1;
										}
										else if($pdfHere == True){
									...
									else if(strcmp(mb_strtolower($nulti), mb_strtolower($word)) > 0){
				...
				<tr style="background-color: #F8F8F8;">
					...
					<?php
						...
						foreach($split_strings as $e){
							if((strcmp($word.".", $e) == 0) || (strcmp($word."!", $e) == 0) || (strcmp($word."?", $e) == 0) || (strcmp($word."?!", $e) == 0)){
								$counterEnd++;
							}
							else if((strcmp($word."?", $e) == 0) || (strcmp($word."!", $e) == 0) || (strcmp($word."?", $e) == 0) || (strcmp($word."?!", $e) == 0)){
								$counterEnd++;
							}
							else if((strcmp($word."!", $e) == 0) || (strcmp($word."!", $e) == 0) || (strcmp($word."?", $e) == 0) || (strcmp($word."?!", $e) == 0)){
								$counterEnd++;
