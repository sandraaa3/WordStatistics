		...
		$(document).ready(function() {
			...
			{ 
				...
				{ 
					...
					//$('#upload').post("compare_two_files_analysis.php");
					showLoading();
					$.ajax({
						type: "POST",
		...
		$(document).ready(function() {
			$('#upload2').bind("click",function() 
				{ 
					...
					if(val=='' && val2=='') 
					{ 
						alert("Empty input FILE 1 and FILE 2 with specific font!"); 
					} 
					else if(val=='' && val2!='') 
					{ 
						alert("Empty input FILE 1  with specific font!"); 
					}
					else if(val!='' && val2=='') 
					{ 
						alert("Empty input FILE 2  with specific font!"); 
					}
					else{
						//$('#upload2').post("compare_two_files_analysis_utf_8.php");
						showLoading();
						$.ajax({
							type: "POST",
						...
						<div class="panel panel-info" id="div_special_font">
						  <div class="panel-body" style="padding-left: 40px; padding-bottom: 40px;">
								<form action="compare_two_files_analysis_utf_8.php" method="post" enctype="multipart/form-data" style="float: left;">
								  ...
								  Choose two files with specific font:</b></h3></label>
								  <br><br>
